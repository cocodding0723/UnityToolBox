...
namespace UnityToolBox.Extension
{
    public class MeshCut
    {
        ...
        // Gathering values
        private static readonly List<int>[] left_Gather_subIndices = {new List<int>(), new List<int>()};
        private static readonly List<int>[] right_Gather_subIndices = {new List<int>(), new List<int>()};

        private static readonly List<Vector3>[] left_Gather_added_Points = {new List<Vector3>(), new List<Vector3>()};
        private static readonly List<Vector2>[] left_Gather_added_uvs = {new List<Vector2>(), new List<Vector2>()};
        private static readonly List<Vector3>[] left_Gather_added_normals = {new List<Vector3>(), new List<Vector3>()};

        private static readonly List<Vector3>[] right_Gather_added_Points = {new List<Vector3>(), new List<Vector3>()};
        private static readonly List<Vector2>[] right_Gather_added_uvs = {new List<Vector2>(), new List<Vector2>()};
        private static readonly List<Vector3>[] right_Gather_added_normals = {new List<Vector3>(), new List<Vector3>()};
        ...
        // final arrays
        private static readonly List<int>[] left_Final_subIndices = {new List<int>(), new List<int>()};
        ...
        private static readonly List<int>[] right_Final_subIndices = {new List<int>(), new List<int>()};
        ...
        // capping stuff
        private static readonly List<Vector3> createdVertexPoints = new List<Vector3>();
            ...
            Material capMaterial)
        {
            ...
            var sub = 0;
            var indices = victim_mesh.triangles;
            var secondIndices = victim_mesh.GetIndices(1);
            ...
            for (var i = 0; i < indices.Length; i += 3)
            {
                ...
                sub = 0;
                for (var k = 0; k < secondIndices.Length; k++)
            ...
            Material[] mats = {victim.GetComponent<MeshRenderer>().material, capMaterial};

            var leftSideObj = victim;

            var rightSideObj = new GameObject("rightSide", typeof(MeshFilter), typeof(MeshRenderer));
        ...
        private static void Cut_this_Face(int submesh, int index1, int index2, int index3)
        {
            ...
            var newVertex1 = Vector3.Lerp(leftPoint1, rightPoint1, normalizedDistance);
            var newUv1 = Vector2.Lerp(left_uv1, right_uv1, normalizedDistance);
            var newNormal1 = Vector3.Lerp(left_normal1, right_normal1, normalizedDistance);
            ...
            var newVertex2 = Vector3.Lerp(leftPoint2, rightPoint2, normalizedDistance);
            var newUv2 = Vector2.Lerp(left_uv2, right_uv2, normalizedDistance);
            var newNormal2 = Vector3.Lerp(left_normal2, right_normal2, normalizedDistance);

            createdVertexPoints.Add(newVertex2);

            // first triangle
            Add_Left_triangle(submesh, newNormal1, new[] {leftPoint1, newVertex1, newVertex2},
                new[] {left_uv1, newUv1, newUv2},
                new[] {left_normal1, newNormal1, newNormal2});

            // second triangle
            Add_Left_triangle(submesh, newNormal2, new[] {leftPoint1, leftPoint2, newVertex2},
                new[] {left_uv1, left_uv2, newUv2},
                new[] {left_normal1, left_normal2, newNormal2});

            // first triangle
            Add_Right_triangle(submesh, newNormal1, new[] {rightPoint1, newVertex1, newVertex2},
                new[] {right_uv1, newUv1, newUv2},
                new[] {right_normal1, newNormal1, newNormal2});

            // second triangle
            Add_Right_triangle(submesh, newNormal2, new[] {rightPoint1, rightPoint2, newVertex2},
                new[] {right_uv1, right_uv2, newUv2},
                new[] {right_normal1, right_normal2, newNormal2});
        }

        private static void Add_Left_triangle(int submesh, Vector3 faceNormal, Vector3[] points, Vector2[] uvs,
            Vector3[] normals)
        {
            var p1 = 0;
            var p2 = 1;
            ...
            var calculated_normal =
                Vector3.Cross((points[1] - points[0]).normalized, (points[2] - points[0]).normalized);
        ...
        private static void Add_Right_triangle(int submesh, Vector3 faceNormal, Vector3[] points, Vector2[] uvs,
            Vector3[] normals)
        {
            var p1 = 0;
            var p2 = 1;
        ...
        private static void SetFinalArrays_withOriginals()
        {
            ...
            {
                for (var i = 0; i < left_Gather_subIndices[submesh].Count; i++)
                {
                ...
                for (var i = 0; i < right_Gather_subIndices[submesh].Count; i++)
                {
        ...
        private static void AddNewTriangles_toFinalArrays()
        {
            for (var submesh = 0; submesh < 2; submesh++)
            {
                var count = left_Final_vertices.Count;
                // add the new ones
                for (var i = 0; i < left_Gather_added_Points[submesh].Count; i++)
                {
                ...
                for (var i = 0; i < right_Gather_added_Points[submesh].Count; i++)
                {
        ...
        private static readonly List<Vector3> capVertTracker = new List<Vector3>();
        private static readonly List<Vector3> capVertpolygon = new List<Vector3>();

        private static void MakeCaps()
        {
            capVertTracker.Clear();

            for (var i = 0; i < createdVertexPoints.Count; i++)
                if (!capVertTracker.Contains(createdVertexPoints[i]))
                {
                    ...
                    while (!isDone)
                    {
                        ...
                        for (var k = 0; k < createdVertexPoints.Count; k += 2)
                            // go through the pairs

                            if (createdVertexPoints[k] == capVertpolygon[capVertpolygon.Count - 1] &&
                                !capVertTracker.Contains(createdVertexPoints[k + 1]))
                            {
                                // if so add the other
                            ...
                            else if (createdVertexPoints[k + 1] == capVertpolygon[capVertpolygon.Count - 1] &&
                                     !capVertTracker.Contains(createdVertexPoints[k]))
                            {
                                // if so add the other
        ...
        private static void FillCap(List<Vector3> vertices)
        {
            ...
            var left = Vector3.Cross(blade.normal, upward);
            ...
            var calculated_normal = Vector3.zero;
            var otherIndex = 0;
            for (var i = 0; i < vertices.Count; i++)
            {
            ...
            for (var i = 0; i < normals.Count; i++) normals[i] = -normals[i];
